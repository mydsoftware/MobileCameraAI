...
state = {"connected": False, "authenticated": False, "packets": 0, "bytes": 0,
         ...
         "flv_header_ok": False, "stream_clients": 0, "ffmpeg": "starting",
         "decoded_frames": 0}
...
<style>body{background:#111;color:#eee;font-family:Arial;margin:20px}img,video{width:min(100%,1280px);background:#000;display:block}pre{background:#222;padding:12px}</style></head>
<body><h2>Uniview Live</h2><img id="m" src="/live.mjpg" alt="loading..."><pre id="s">connecting...</pre>
<script>
async function st(){try{let r=await fetch('/status');document.getElementById('s').textContent=JSON.stringify(await r.json(),null,2)}catch(e){}}
    ...
    try:
        ...
        while True:
            ...
            with stream_lock:
                ...
                if state["packets"] == 1: parse_flv_tags(bytes(stream_prefix))
            feed_ffmpeg(packet)
            broadcast(packet)
    except Exception as e:
...
def live_flv():
    ...
    c=Client()
    with stream_lock: prefix=bytes(stream_prefix)
...
def main():
    print('='*70); print('Uniview WebSocket → FLV → FFmpeg → MJPEG'); print('Endpoint:',WS_URL); print('='*70)
