import os,json,re,hashlib,shutil,subprocess,threading,time,queue
import websocket
...
PRESETS={'Camera 1 / Main':(8001,'/media/flv/video1'),'Camera 1 / Sub':(8001,'/media/flv/video2'),'Camera 1 / Third':(8001,'/media/flv/video3'),'Camera 2 / Main':(8002,'/media/flv/video1'),'Camera 2 / Sub':(8002,'/media/flv/video2'),'Camera 2 / Third':(8002,'/media/flv/video3'),'Camera 2 / Main media2':(8002,'/media2/flv/video1'),'Camera 2 / Sub media2':(8002,'/media2/flv/video2'),'Camera 2 / Third media2':(8002,'/media2/flv/video3'),'Camera 3 / Main':(8003,'/media3/flv/video1'),'Camera 3 / Sub':(8003,'/media3/flv/video2'),'Camera 3 / Third':(8003,'/media3/flv/video3')}
...
def ffmpeg_loop():
 ...
 try:
  ...
  if not binary:raise RuntimeError('ffmpeg not found')
  proc=subprocess.Popen([binary,'-hide_banner','-loglevel','warning','-fflags','nobuffer+genpts','-flags','low_delay','-f','flv','-i','pipe:0','-an','-vf','fps=20,scale=1280:-2:force_original_aspect_ratio=decrease','-c:v','mjpeg','-q:v','8','-flush_packets','1','-f','mpjpeg','-boundary_tag','frame','pipe:1'],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,bufsize=0);state['ffmpeg']='running'
  def err():
...
def start(p,path):
 global ws
 stop.set();time.sleep(.3);stop_proc();stop.clear();state.update(connected=False,authenticated=False,streaming=False,ffmpeg='starting',packets=0,bytes=0,frames=0,last_error='',port=p,path=path)
 def run():
  global ws
  while not stop.is_set():
   try:
    threading.Thread(target=ffmpeg_loop,daemon=True).start();ws=connect(p,path);state.update(connected=True,authenticated=True,streaming=True)
    while not stop.is_set():
     z=ws.recv()
     if isinstance(z,str):
      if '"errorCode":401' in z:raise RuntimeError(z)
      continue
     if not z:raise RuntimeError('WebSocket closed')
     state['packets']+=1;state['bytes']+=len(z)
     if proc and proc.stdin:
      proc.stdin.write(z)
   except Exception as e:
    if not stop.is_set():state.update(streaming=False,last_error=repr(e));time.sleep(1)
   finally:
    try:
     if ws:ws.close()
    except:pass
...
class Client:
 def __init__(self):self.cv=threading.Condition();self.latest=None;self.closed=False
 def push(self,x):
